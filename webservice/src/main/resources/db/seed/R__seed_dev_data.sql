-- Local-dev-only sample data: venues, media locations, media screens, and bundles.
--
-- Scope is deliberately supply-side only. business/employee are NOT seeded here because
-- employee.user_id is the raw Auth0 JWT `sub` used for every authorization check
-- (see JwtUtils) — a fabricated value would just make the seeded business invisible to
-- any dev logged in with their own Auth0 account. Create your business/campaign through
-- the normal signup flow in the running app instead; this migration exists to give that
-- flow something real to browse (media to select, bundles to subscribe to) without
-- requiring a second developer to have set up a media-owner account first.
--
